...
const Tr = styled.tr`
  ...
  border-radius: 6px;
  box-shadow: ${({ highlighted }) => (highlighted ? '0 0 6px #faff53' : 'none')};
  cursor: pointer;
    ...
    <Wrapper align="center" justify="space-between">
      <TableWrapper>
        <StyledTable>
          <Thead>
            <tr>
              <SortableTh onClick={() => handleSort('label')}>EXCHANGE</SortableTh>
              <SortableTh onClick={() => handleSort('quote')}>QUOTE</SortableTh>
              <SortableTh onClick={() => handleSort('type')}>MIN. RECEIVED</SortableTh>
              <SortableTh onClick={() => handleSort('gasFee')}>GAS FEE</SortableTh>
              <SortableTh onClick={() => handleSort('save')}>YOU SAVE</SortableTh>
            </tr>
          </Thead>
          <tbody>
            {sortedData.map((ex, index) => (
              <Tr key={index} highlighted={ex.isHighlighted} onClick={() => handleRowClick(ex)}>
                ...
                <Td>{ex.quote}</Td>
                <Td><PriceTag>{ex.type}</PriceTag></Td>
                <Td><GasIcon />{ex.gasFee}</Td>
                <Td>{ex.save}</Td>
              </Tr>
            ))}
          </tbody>
